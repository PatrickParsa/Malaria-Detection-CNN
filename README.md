...
In this project, I built several different convolutional neural network models in an attempt to accurately classify images of red blood cells as either parasitized or uninfected. I used an array of different techniques to better the performance from the base model (three convolutional layers each followed by max pooling and dropout layers) including adding more layers, batch normalization, changing activation functions, image alterations, and hyperparameter tuning using Keras Tuner. **The best model ended up being our base model with one added convolutional layer followed by an additional max pooling layer and dropout layer**. I then improved the performance by finding the optimal hyperparameters using Keras Tuner, and then found that feeding this model grayscaled images allowed stronger classification. 
...
Given that convolutional neural networks only accept 4D arrays, I had to convert the images accordingly, and then I also normalized the images by dividing each image by the max pixel count (255) which makes the data much more optimal for our models. It makes training faster and reduces the chances of getting stuck at a local minima.

## Model building

Before initializing any models, it was important that I establish the metric that is most important to maximize. In this case, I wanted to minimize the number of misclassified infected cells, which means that I want to obtain the **highest recall score** for the infected class. This is because unnoticed infections would be damaging to the overarching goal of reducing the impact of malaria. 

### Base Model 

The base model contained 3 convolutional layers, each followed by a **MaxPooling2D** and **Dropout** layer. 
...
The activation function I used is ReLU (Rectified Linear Unit), which overcomes the vanishing gradient problem allowing the models to learn faster and perform better. It is a piecewise linear function that will output the input directly if it is positive, otherwise, it will output zero. 
...
I got a strong performance from the base model, with high precision, recall, and f-1 scores as can be seen from the confusion matrix above. In particular, I focused on the recall score of our infected class, and we can see that in this case it is a 94% score and a digit of 73. This means that only 73 images were misclassified as being uninfected when they are actually parasitizied. I then tried to improve these scores even further. 

### Improvements to base model

Our second model simply involved adding another convolutional layer folloewd by a MaxPooling2D and Dropout layer. I saw a very slight drop in performance for recall on the infected class however I did see significant improvement on the recall score of the uninfected class, from 64 misclassifications to 77. 
...
In this phase, I decided to add **Batch Normalization** and changed the activation function to **LeakyReLU** to see if I could improve the performance. But first, it is important to understand what these changes actually do. 
...
I saw a decrease in performance, as there is a higher number of misclassifications for our infected class compared to our initial class. 

## Pre-Trained model 

I tried using a pre-trained model to see how it would perform on our dataset. The model in particular that we used is **VGG16**. 
...
Despite a strong performance, it did not have improved scores compared to the already-strong base model. Thus, I moved forward with the base model to see how I can further improve it. 

## Keras Tuner

One robust way to way to improve a TensorFlow convolutional neural network is by using the **Keras Tuner** which is a library that can help pick the optimal set of hyperparameters for our model. It works by giving the tuner a set of options for values in different hyperparameters and then it automatically tests different sets of values and combinations, evaluates their performance, and then provides a list of results. I can then tune our paramaters using the ones provided by the top scoring values given in the list of results. 
...
Even though it's only slight, I did see an improvement from my best model so far since there are fewer misclassifications of the infected class (71). 

## Image alterations

In this phase, I shifted our focus towards the dataset itself rather than our model. This is to see if any alterations to the images make them easier to classify for my best model. 
...
Despite strong performance for the uninfected class, I saw poor performance for the infected class which is what we are most interested in. 

### HSV conversion

HSV or Hue Saturation Value is used to seperate image luminance from color information. By converting the images to HSV, my model might have an easier time classifying the images. 
...
Interestingly, I saw an extremely strong performance for our targeted recall score of the infected class (only 46 misclassifications) however the model performed very poorly on all other metrics using these images. Therefore, I did not move forward with this model. 

### Grayscale

Grayscale conversion might help remove some of the noise that may be contributing to the misclassifications of the model. 
...
The metrics show that this was the best performance. As indicated by the green circle, the number of misclassified infected cells is only 68 while the rest of the performance metrics all remain very strong. 

## Conclusion:

In this project, I explored a number of ways for building a deep learning model that can
...
of a pre-trained model. After trialing these techniques, I was able to identify a model that
provides a balanced performance, meaning that the accuracy metrics were all relatively high.
Despite obtaining a strong model, I did not exhaust all the available options available to
...
I could could simply alter some of its features to fit this problem. 

Additionally, I can actually display the cells that are being misclassified and try to understand why, and then consult with
medical experts to obtain a better understanding or even have them correct any potential
misclassified labels of the dataset. After these improvements, my next step would be to begin
deployment of the model, so that I can use its capabilities in real time. This would
involve communication with the engineering team to create a pipeline which would grant us
greater control and flexibility for our model, as well as scalability. This is important because I
was only dealing with a single dataset for our current model whereas I will want it to be able to process much more data and in real time, so if there are issues then I will want to be able to
quickly fix it and a pipeline will help greatly with that.

## Problem and Solution Summary

The main challenge was being able to distinguish between red blood cells that are
infected, and those that are not. Different features such as color, opacity, and other types of noise
can make it difficult for my automated model to make this distinction. Thus, my main objective
was to utilize different techniques to both equip my model to be more effective at this task and
also alter the images in a way to make this goal less challenging. I tried various techniques
such as HSV conversion and augmentation for the images, as well as adding features to our
model such as batch normalization. My final proposed solution design involves grayscale
images being fed into a keras-tuned model. The grayscale conversion helped eliminate some of
the noise of the images that may have contributed to the misclassifications,
while the keras tuner helped obtain the best hyperparameters needed for the model to work
optimally with the data.

## Next Steps

The grayscale technique helped make the images easier to classify for the model,
...
infected cell but can only be distinguished by its colour, then the model might fail to make that
distinction. Thus, communication with medical experts on this topic might help better explore this potential limitation and thus create contingencies that will help prevent it from being a
...
such as Bayesian optimization may produce better results because it would allow me to jointly tune
more parameters with fewer experiments and find better values, all at a potentially quicker speed.
The slow speeds of our tuning might also mean that I should consult with the engineering team
to make sure an appropriate environment can be established so that when further potential tuning
happens down the line, I won’t be limited by slow processing speeds. This is especially
important considering I will be working with live data and high amounts of it. Thus, consultation
...
importance. Following that, I can explore different tuning methods such as bayesian or grid
search to find even better hyperparameters than what I found with the Keras tuner.
